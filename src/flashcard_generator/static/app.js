...
async function generateCards() {
  ...
  setBusy(true);
  showNotice(useAI ? "Preparing the local T5 question model. First use downloads about 900 MB automatically." : "Processing locally with rules only.");
  try {
...
function applyResult(data) {
  ...
  $("#deck-mode").textContent = data.mode === "ai" ? "Generated locally by T5 · " + data.model + " · " + (data.device || "automatic device") : "Generated by local rules · no AI";
...
function renderCoverage() {
  ...
  if (!coverage) return;
  const covered = coverage.covered_units ?? coverage.covered_facts;
  const identified = coverage.identified_units ?? coverage.identified_facts;
  $("#coverage-summary").textContent = covered + " / " + identified + " source units covered";
  $("#coverage-state").textContent = "Generation-time validation. " + coverage.requests + " local inference passes.";
  const items = [
...
modeInput.addEventListener("change", () => {
  $("#mode-help").textContent = modeInput.value === "ai"
    ? "Runs on your computer with a question-generation T5 model. First use downloads about 900 MB; answers stay exact source excerpts."
    : "Pattern matching only. No AI review or coverage audit. Nothing leaves this app.";
});
...
$("#cancel-button").addEventListener("click", async () => {
  ...
  try {
    ...
    showNotice("Cancellation requested. Waiting for the current model pass to finish.");
    if (!busy) {
      setBusy(true);
      try { await followJob(); }
