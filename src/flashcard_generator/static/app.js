...
async function generateCards() {
  ...
  setBusy(true);
  showNotice(useAI ? "Starting local coverage audit through Ollama." : "Processing locally with rules only.");
  try {
...
function applyResult(data) {
  ...
  $("#deck-mode").textContent = data.mode === "ai" ? "Generated and reviewed locally by Ollama · " + data.model : "Generated by local rules · no AI";
...
modeInput.addEventListener("change", () => {
  $("#mode-help").textContent = modeInput.value === "ai"
    ? "Runs on your computer through Ollama. Inventories facts, audits omissions, drafts and reviews cards. Multiple local passes can take several minutes."
...
$("#cancel-button").addEventListener("click", async () => {
  ...
  try {
    ...
    showNotice("Cancellation requested. Waiting for the current Ollama call to finish.");
    if (!busy) {
      setBusy(true);
      try { await followJob(); }
